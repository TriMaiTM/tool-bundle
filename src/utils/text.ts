...
export function removeDuplicateLines(text: string, options: RemoveDuplicateOptions = {}): string {
	if (!text) return "";
	const { caseSensitive = true, trimWhitespace = true, keepEmptyLines = true } = options;
	const lines = text.split("\n");
...
export type SortMode = "alphabetical" | "reverse" | "shortest" | "longest" | "random";

export function sortLines(text: string, mode: SortMode, caseSensitive = true): string {
	if (!text) return "";
	const lines = text.split("\n");
	const sorted = [...lines];
