...
export function arrayToCSV(data: Record<string, unknown>[], delimiter = ","): string {
...
export function formatCSV(text: string, delimiter = ","): string {
	const rows = parseCSV(text, delimiter);
	return rows.map((row) => row.join(delimiter)).join("\n");
}

export function validateCSV(text: string, delimiter = ","): { valid: boolean; errors: string[] } {
	...
	for (let i = 0; i < rows.length; i++) {
		if (rows[i].length !== expectedCols) {
			errors.push(`Line ${i + 1}: expected ${expectedCols} columns, found ${rows[i].length}`);
		}
	}
