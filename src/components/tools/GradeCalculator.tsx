...
export default function GradeCalculator() {
	...
	const whatIf = useMemo(() => {
		...
		const fw = Number.parseFloat(finalWeight);
		if (Number.isNaN(fw) || fw <= 0 || fw >= 100) return null;
		...
		return {
			needed: neededFinal,
			achievable: neededFinal >= 0 && neededFinal <= 100,
		};
		...
		<div class="space-y-6">
			...
			<div class="bg-surface-elevated rounded-lg p-3">
				<h3 class="text-title-lg text-primary mb-4">
					{editId ? "Edit Entry" : "Add Course / Assignment"}
				</h3>
				...
				<div class="bg-surface-elevated rounded-lg p-3">
					...
					<div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
						...
						<div class="flex items-end">
							{whatIf && (
								<div
									class={`p-3 rounded-lg ${whatIf.achievable ? "bg-accent-emerald/10 text-accent-emerald" : "bg-accent-rose/10 text-accent-rose"}`}
								>
									<div class="text-body-strong">Need: {whatIf.needed.toFixed(1)}%</div>
