...
const BODY_FAT_CATEGORIES: BodyFatCategory[] = [
  { label: "Essential Fat", maleRange: "2–5%", femaleRange: "10–13%", color: "text-accent-blue" },
  { label: "Athletes", maleRange: "6–13%", femaleRange: "14–20%", color: "text-accent-emerald" },
  { label: "Fitness", maleRange: "14–17%", femaleRange: "21–24%", color: "text-accent-emerald" },
  { label: "Average", maleRange: "18–24%", femaleRange: "25–31%", color: "text-warning" },
  { label: "Obese", maleRange: "25%+", femaleRange: "32%+", color: "text-accent-rose" },
];
...
export default function BodyFatCalculator() {
  ...
  const convertToCm = useCallback(
    (value: string, unit: CircUnit): number => {
  ...
  const result = useMemo(() => {
    ...
    if (isNaN(bfPct) || bfPct < 0 || bfPct > 70) return null;
  ...
  const handleCircUnitChange = useCallback(
    (unit: CircUnit) => {
      const convert = (val: string) => {
        ...
        if (isNaN(v) || val.trim() === "") return val;
                  ...
                  return (
                    ...
                    >
                      <div class="flex items-center justify-between">
                        <span class={`text-body-sm ${isActive ? "text-body-strong" : "text-muted"}`}>
                          {cat.label}
                        </span>
