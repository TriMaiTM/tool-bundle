...
export default function JavaScriptFormatter() {
  ...
  const sizeComparison = useMemo(() => {
    ...
    if (diff > 0)
      return `${pct}% smaller (${formatBytes(inputSize)} → ${formatBytes(outputSize)})`;
    if (diff < 0)
          ...
          <>
            ...
            <label class="flex items-center gap-2 text-body-sm cursor-pointer">
              <input
                ...
                onChange={(e) =>
                  setUppercaseKeywords((e.target as HTMLInputElement).checked)
                }
              />
              Uppercase Keywords
            </label>
        ...
        <div>
          <div class="flex items-center justify-between mb-2">
            <label class="text-caption-uppercase text-muted">
              Output
              {sizeComparison && (
                <span class="badge badge-yellow ml-2">{sizeComparison}</span>
              )}
            </label>
            {result && (
              <button
