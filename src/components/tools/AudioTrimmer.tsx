...
export default function AudioTrimmer() {
  ...
  const [audioInfo, setAudioInfo] = useState<{ name: string; size: string; duration: string } | null>(null);
  ...
  const handlePreviewTrimmed = useCallback(() => {
    ...
    if (playbackRef.current) {
      try { playbackRef.current.source.stop(); } catch {}
      try { playbackRef.current.ctx.close(); } catch {}
    }
        ...
        <div>
          ...
          {audioUrl && (
            <div class="bg-surface-elevated rounded-lg p-3 mb-4">
              ...
              {/* Visual progress bar */}
              <div class="mt-3" style="height: 6px; background: var(--color-surface); border-radius: 3px; overflow: hidden">
                <div
                  class="badge-yellow"
          ...
          {audioInfo && (
            <div class="bg-surface-elevated rounded-lg p-3 mb-4">
              ...
              <div class="text-body-sm text-primary">
                <div><strong>Name:</strong> {audioInfo.name}</div>
                <div><strong>Size:</strong> {audioInfo.size}</div>
          ...
          {trimmedBlob && (
            <div class="bg-surface-elevated rounded-lg p-3 mb-4">
              ...
              <div class="text-body-sm text-primary">
                <div><strong>Duration:</strong> {formatTime(parseTime(endTime) - parseTime(startTime))}</div>
                <div><strong>Size:</strong> {formatFileSize(trimmedBlob.size)}</div>
              </div>
              <audio
