...
export default function VolumeBooster() {
  ...
  const previewRef = useRef<{ ctx: AudioContext; source: AudioBufferSourceNode; gain: GainNode } | null>(null);
        ...
        <div>
          ...
          {boostedBlob && (
            <div class="bg-surface-elevated rounded-lg p-3 mb-4">
              ...
              <div class="text-body-sm text-primary">
                <div><strong>Volume:</strong> {volume}% ({gainX}x)</div>
                <div><strong>Size:</strong> {formatFileSize(boostedBlob.size)}</div>
              </div>
              <audio
                src={URL.createObjectURL(boostedBlob)}
                controls
                class="mt-3 w-full"
              />
            </div>
