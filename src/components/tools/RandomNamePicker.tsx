...
export default function RandomNamePicker() {
  ...
  return (
    <div>
      <div class="mb-6">
        <label class="text-caption-uppercase text-muted block mb-2">Paste Names (one per line)</label>
        <textarea
        ...
        />
        <button class="btn-secondary mt-2" onClick={importFromText}>Import Names</button>
      </div>
      ...
      <div class="mb-6">
        ...
        <div class="flex gap-2">
          <input
            ...
            onKeyDown={(e: KeyboardEvent) => { if (e.key === "Enter") addName(); }}
          />
          <button class="btn-secondary" onClick={addName}>Add</button>
        </div>
      </div>
      ...
      {names.length > 0 && (
        <div class="mb-6">
          <div class="flex items-center justify-between mb-3">
            ...
            <div class="flex gap-2">
              <button class="btn-secondary text-body-sm" onClick={shuffleList}>Shuffle</button>
              <button class="btn-secondary text-body-sm" onClick={clearAll}>Clear All</button>
            </div>
          </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        <div>
          ...
          <input
            ...
            min={1}
            max={10}
            onInput={(e) => setWinnerCount(Math.min(10, Math.max(1, parseInt((e.target as HTMLInputElement).value) || 1)))}
          />
        </div>
      ...
      {picking && (
        <div class="bg-surface-elevated rounded-lg p-6 mb-6 text-center">
          <div class="text-title-lg text-primary" style="font-size: 2rem; animation: pulse 0.2s ease-in-out infinite alternate;">
            {displayedName}
          </div>
          <style>{`
            @keyframes pulse {
      ...
      {winners.length > 0 && !picking && (
        <div class="bg-surface-elevated rounded-lg p-6 mb-6 text-center">
          <span class="text-caption-uppercase text-muted block mb-3">🎉 Winner{winners.length > 1 ? "s" : ""}!</span>
          {winners.map((w, i) => (
                ...
                <div class="flex items-center justify-between">
                  <div class="flex flex-wrap gap-1">
                    {entry.winners.map((w, j) => (
                      <span class="badge-yellow" key={j}>{w}</span>
                    ))}
                  </div>
