...
export default function SentimentAnalysis() {
  ...
  const handleAnalyze = useCallback(async () => {
    ...
    try {
      ...
      const mapped = (Array.isArray(output) ? output : [output]).map(
        (item: any) => ({
      ...
      setError(
        err instanceof Error
          ? err.message
          : "Failed to analyze sentiment. Try shorter text.",
      );
      setStatus("error");
    }
  }, [input]);
  ...
  const overallScore = overallSentiment
    ? Math.max(overallSentiment.positive, overallSentiment.negative) /
      results.length
    : 0;
              ...
              <div class="flex items-center gap-4">
                ...
                <div>
                  <div
                    ...
                    style={{
                      color:
                        overallLabel === "POSITIVE" ? "#22c55e" : "#ef4444",
                    }}
                  >
                    {overallLabel}
                  </div>
                ...
                {results.map((r, i) => (
                  ...
                  >
                    <span
                      ...
                      style={{
                        color:
                          r.label === "POSITIVE" ? "#22c55e" : "#ef4444",
                      }}
                    >
                      {r.label === "POSITIVE" ? "✅" : "❌"}
                    </span>
