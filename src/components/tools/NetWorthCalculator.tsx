...
const ASSET_CATEGORIES: AssetCategory[] = ["Cash", "Investments", "Real Estate", "Vehicles", "Other"];
const LIABILITY_CATEGORIES: LiabilityCategory[] = ["Mortgage", "Student Loans", "Credit Cards", "Car Loan", "Other"];
...
export default function NetWorthCalculator() {
  ...
  const handleAddAsset = useCallback(() => {
    ...
    setAssets((prev) => [...prev, { id: generateId(), name: assetName.trim(), value: val, category: assetCategory }]);
    setAssetName("");
    setAssetValue("");
  }, [assetName, assetValue, assetCategory]);

  const handleAddLiability = useCallback(() => {
    const val = parseFloat(liabilityValue);
    if (!liabilityName.trim() || isNaN(val) || val <= 0) return;
    setLiabilities((prev) => [...prev, { id: generateId(), name: liabilityName.trim(), value: val, category: liabilityCategory }]);
    setLiabilityName("");
    setLiabilityValue("");
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div>
            ...
            <select
              class="input w-full"
              value={assetCategory}
              onChange={(e) => setAssetCategory((e.target as HTMLSelectElement).value as AssetCategory)}
            >
          ...
          <div class="flex items-end">
            <button class="btn-primary w-full" onClick={handleAddAsset}>Add Asset</button>
          </div>
        </div>
      </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => setLiabilityCategory((e.target as HTMLSelectElement).value as LiabilityCategory)}
            >
          ...
          <div class="flex items-end">
            <button class="btn-primary w-full" onClick={handleAddLiability}>Add Liability</button>
          </div>
        </div>
      </div>
                ...
                return (
                  <div key={category}>
                    <div class="flex justify-between text-body-sm mb-1">
                      ...
                      <span class="text-accent-emerald">${formatCurrency(value)} ({pct.toFixed(1)}%)</span>
                    </div>
              ...
              {liabilityByCategory.map(({ category, value }) => {
                ...
                return (
                  <div key={category}>
                    <div class="flex justify-between text-body-sm mb-1">
                      <span class="text-muted">{category}</span>
                      <span class="text-accent-rose">${formatCurrency(value)} ({pct.toFixed(1)}%)</span>
                    </div>
                    <div class="h-4 rounded overflow-hidden bg-surface-soft">
                      <div
                        class="h-full bg-accent-rose rounded"
        ...
        {assets.length === 0 ? (
          ...
          <div class="overflow-x-auto">
            <table class="w-full text-body-sm">
              ...
              <tbody>
                {assets.map((a) => (
                  <tr key={a.id} class="border-b border-hairline/50 hover:bg-surface-soft/50 transition-colors">
                    <td class="py-2 px-3 text-body-strong">{a.name}</td>
                    <td class="py-2 px-3"><span class="badge">{a.category}</span></td>
                    <td class="py-2 px-3 text-right text-accent-emerald font-medium">${formatCurrency(a.value)}</td>
                    <td class="py-2 px-3 text-right">
                      <button class="btn-secondary text-body-sm" onClick={() => handleDeleteAsset(a.id)}>Delete</button>
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        {liabilities.length === 0 ? (
          ...
          <div class="overflow-x-auto">
            <table class="w-full text-body-sm">
              ...
              <tbody>
                {liabilities.map((l) => (
                  <tr key={l.id} class="border-b border-hairline/50 hover:bg-surface-soft/50 transition-colors">
                    <td class="py-2 px-3 text-body-strong">{l.name}</td>
                    <td class="py-2 px-3"><span class="badge">{l.category}</span></td>
                    <td class="py-2 px-3 text-right text-accent-rose font-medium">${formatCurrency(l.value)}</td>
                    <td class="py-2 px-3 text-right">
                      <button class="btn-secondary text-body-sm" onClick={() => handleDeleteLiability(l.id)}>Delete</button>
                    </td>
                  </tr>
                ))}
              </tbody>
