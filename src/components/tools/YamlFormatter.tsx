		...
		<div>
			...
			{result.error && (
				<div class="bg-accent-rose/10 border border-accent-rose/30 rounded-lg p-4 mb-4 text-body-sm text-accent-rose">
					<strong>Invalid YAML{result.errorLine ? ` (line ${result.errorLine})` : ""}:</strong>{" "}
					{result.error}
				</div>
			)}
				...
				<div>
					<div class="flex items-center justify-between mb-2">
						<label class="text-caption-uppercase text-muted">
							{mode === "format" ? "Formatted Output" : "Validation Result"}
						</label>
					...
					<textarea
						...
						readOnly
						placeholder={
							mode === "format" ? "Formatted YAML will appear here..." : "Paste YAML to validate..."
						}
					/>
				</div>
			</div>
