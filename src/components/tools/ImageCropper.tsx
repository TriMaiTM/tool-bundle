...
import { cropImage, loadImage, type ImageFormat } from "../../utils/image";
        ...
        <div>
              ...
              {ASPECT_PRESETS.map((p) => (
                <button
                  ...
                  class={`px-3 py-1.5 rounded-md text-body-sm font-medium transition-colors ${
                    aspectRatio === p.ratio ? "bg-primary text-on-primary" : "bg-surface-elevated text-body hover:text-on-dark"
                  }`}
          ...
          <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
            <div>
              ...
              <input type="number" class="input" min={0} max={imgWidth - cropWidth} value={x} onInput={(e) => handleXChange(Number((e.target as HTMLInputElement).value) || 0)} />
            </div>
            <div>
              <label class="text-caption text-muted block mb-1">Y Position</label>
              <input type="number" class="input" min={0} max={imgHeight - cropHeight} value={y} onInput={(e) => handleYChange(Number((e.target as HTMLInputElement).value) || 0)} />
            </div>
            <div>
              <label class="text-caption text-muted block mb-1">Width</label>
              <input type="number" class="input" min={1} max={imgWidth - x} value={cropWidth} onInput={(e) => handleCropWidthChange(Number((e.target as HTMLInputElement).value) || 1)} />
            </div>
            <div>
              <label class="text-caption text-muted block mb-1">Height</label>
              <input type="number" class="input" min={1} max={imgHeight - y} value={cropHeight} onInput={(e) => handleCropHeightChange(Number((e.target as HTMLInputElement).value) || 1)} />
            </div>
          </div>
          ...
          <div class="flex flex-wrap items-center gap-4 mb-6">
            <label class="flex items-center gap-2 text-body-sm">
              ...
              <select class="input" style="width: auto; height: 36px" value={format} onChange={(e) => setFormat((e.target as HTMLSelectElement).value as ImageFormat)}>
            ...
            {format !== "image/png" && (
              <label class="flex items-center gap-2 text-body-sm">
                ...
                <input type="range" min="10" max="100" value={quality} onInput={(e) => setQuality(Number((e.target as HTMLInputElement).value))} style="width: 120px; accent-color: var(--color-primary)" />
              </label>
            )}
          </div>
