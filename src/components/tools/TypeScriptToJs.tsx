...
function stripTypeScript(code: string): StripResult {
	...
	result = result.replace(/(?:export\s+)?type\s+\w+(?:<[^>]+>)?\s*=\s*[^;]+;/g, (match) => {
		removedCount++;
	...
	result = result.replace(
		/__STRING_(\d+)__/g,
		(_, idx) => stringPlaceholders[Number.parseInt(idx)],
	);

	// Restore comments
	result = result.replace(
		/__COMMENT_(\d+)__/g,
		(_, idx) => commentPlaceholders[Number.parseInt(idx)],
	);
	...
	return (
		<div>
			...
			<div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
				<div>
					<label class="text-caption-uppercase text-muted block mb-2">Input TypeScript</label>
					<textarea
						class="textarea"
				...
				<div>
					<div class="flex items-center justify-between mb-2">
						<label class="text-caption-uppercase text-muted">Output JavaScript</label>
						{result.code && (
							<button
