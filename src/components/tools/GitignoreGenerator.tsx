...
interface TechOption { id: string; name: string; category: string; }
    ...
    <div>
          ...
          <div key={cat} class="mb-3">
            ...
            <div class="flex flex-wrap gap-2">
              {TECHS.filter((t) => t.category === cat).map((tech) => (
                <button key={tech.id} onClick={() => toggle(tech.id)} class={`px-3 py-1.5 rounded-lg text-body-sm transition-all ${selected.has(tech.id) ? "bg-primary text-on-primary" : "bg-surface-elevated text-body hover:text-on-dark border border-hairline"}`}>
                  {tech.name}
                </button>
              ))}
            </div>
      ...
      {result && (
        <div class="mb-4">
          ...
          <pre class="code-block" style="min-height: 200px; max-height: 500px; overflow-y: auto"><code>{result}</code></pre>
        </div>
      )}

      {!result && (
        <div class="bg-surface-elevated rounded-lg p-6 text-center">
          <p class="text-body-sm text-muted">Select technologies above to generate your .gitignore file.</p>
        </div>
      )}
    </div>
  );
