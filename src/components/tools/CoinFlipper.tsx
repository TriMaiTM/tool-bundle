...
export default function CoinFlipper() {
	...
	return (
		<div>
			...
			{flipping && (
				<div class="text-center mb-6">
					<span style="font-size: 4rem; display: inline-block; animation: spin 0.15s linear infinite;">
						🪙
					</span>
					<style>{`
            @keyframes spin {
						...
						<div class="border-t border-hairline pt-3 grid grid-cols-2 gap-2">
							<div>
								...
								<span class="text-body-strong">
									{multiResults.filter((r) => r === "Heads").length}
								</span>
							</div>
							<div>
								<span class="text-body-sm text-muted-soft block">Tails</span>
								<span class="text-body-strong">
									{multiResults.filter((r) => r === "Tails").length}
								</span>
							</div>
						</div>
					</div>
			...
			{total > 0 && (
				<div>
					<div class="flex items-center justify-between mb-3">
						...
						<button class="btn-secondary text-body-sm" onClick={resetStats}>
							Reset
						</button>
					</div>
					<div class="bg-surface-elevated rounded-lg p-3">
						<div class="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
							...
							<div>
								<span class="text-body-sm text-muted-soft block">Heads</span>
								<span class="text-body-strong">
									{stats.heads} ({headsPct.toFixed(1)}%)
								</span>
							</div>
							<div>
								<span class="text-body-sm text-muted-soft block">Tails</span>
								<span class="text-body-strong">
									{stats.tails} ({tailsPct.toFixed(1)}%)
								</span>
							</div>
							<div>
								<span class="text-body-sm text-muted-soft block">Ratio</span>
								<span class="text-body-strong">
									{stats.tails > 0 ? (stats.heads / stats.tails).toFixed(2) : "∞"}
									:1
								</span>
							</div>
						</div>
