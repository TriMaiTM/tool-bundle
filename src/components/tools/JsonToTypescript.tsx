...
function jsonToTs(obj: any, indent = 0, _name = "Root"): string {
...
function generateAllInterfaces(obj: any, name = "Root"): string {
	const parts: string[] = [];
...
export default function JsonToTypescript() {
	...
	const handleConvert = useCallback(() => {
		...
		try {
			...
			const result =
				mode === "interface"
					? generateAllInterfaces(parsed, typeName)
					: `type ${typeName} = ${jsonToTs(parsed)};`;
			setOutput(result);
		} catch (err) {
		...
		<div>
			<div class="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-4">
				<div>
					...
					<textarea
						...
						style="min-height: 300px; font-size: 13px"
				...
				<div>
					...
					<pre
						...
						style="min-height: 300px; font-size: 13px; white-space: pre-wrap"
					>
			...
			<div class="flex flex-wrap items-end gap-3 mb-4">
				...
				<div>
					...
					>
						<button
							class={`px-4 py-2 text-body-sm font-medium ${mode === "interface" ? "bg-primary text-on-primary" : "bg-surface-elevated text-body"}`}
							onClick={() => setMode("interface")}
						>
							Interface
						</button>
						<button
							class={`px-4 py-2 text-body-sm font-medium ${mode === "type" ? "bg-primary text-on-primary" : "bg-surface-elevated text-body"}`}
				...
				<button class="btn-primary" onClick={handleConvert} disabled={!input.trim()}>
					Convert
				</button>
			</div>
