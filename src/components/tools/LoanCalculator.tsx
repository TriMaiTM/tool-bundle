...
export default function LoanCalculator() {
	...
	const calculation = useMemo(() => {
		...
		const r = Number.parseFloat(annualRate);
		...
		const monthlyPayment =
			(p * (monthlyRate * (1 + monthlyRate) ** totalMonths)) /
				...
				<div class="space-y-4">
					...
					<div class="bg-surface-elevated rounded-lg p-6">
						...
						<div class="flex h-6 rounded-full overflow-hidden mb-3">
							<div
								class="bg-primary"
								style={`flex: ${Number.parseFloat(principal)}`}
								title="Principal"
							/>
							<div
						...
						<div class="flex justify-between text-body-sm">
							<div class="flex items-center gap-2">
								...
								<span class="text-muted">
									Principal: ${formatCurrency(Number.parseFloat(principal))} (
									{((Number.parseFloat(principal) / calculation.totalPayment) * 100).toFixed(1)}
									%)
								</span>
							</div>
							<div class="flex items-center gap-2">
					...
					<div class="bg-surface-elevated rounded-lg p-6">
						...
						{showSchedule && calculation.schedule.length === 0 && (
							<p class="text-muted text-body-sm">
								Schedule is too large to display (over 600 months).
							</p>
						)}
					</div>
				</div>
