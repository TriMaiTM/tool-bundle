	...
	useEffect(() => {
		try {
			...
			if (saved) {
				...
				if (parsed?.questions) setQuiz(parsed);
			}
		} catch {
			// ignore
		}
		...
		<div class="space-y-6">
				...
				<div class="space-y-6">
					...
					<div class="bg-surface-elevated rounded-lg p-3">
						<h3 class="text-title-lg text-primary mb-4">
							{editQId ? "Edit Question" : "Add Question"}
						</h3>
						<div class="space-y-4">
							<div>
							...
							<div>
								...
								<select
									...
									value={qCorrect}
									onChange={(e) =>
										setQCorrect(Number.parseInt((e.target as HTMLSelectElement).value))
									}
								>
									{OPTION_LABELS.map((label, i) => (
			...
			{mode === "take" && (
				<div class="space-y-6">
					...
					{quiz.title && <h2 class="text-title-lg text-primary">{quiz.title}</h2>}
						...
						<>
							...
							<div class="flex items-center justify-between">
								...
								{currentQIndex < quiz.questions.length - 1 ? (
									<button class="btn-primary" onClick={() => setCurrentQIndex((prev) => prev + 1)}>
										Next
									</button>
								) : (
					...
					{submitted && score && !reviewMode && (
						<div class="space-y-4">
							<div class="bg-surface-elevated rounded-lg p-6 text-center">
								...
								<div
									class={`badge ${score.passed ? "bg-accent-emerald text-white" : "bg-accent-rose text-white"} text-body-sm`}
								>
								...
								<div class="space-y-2">
									{quiz.questions.map((q, i) => {
										...
										return (
											<div
												key={q.id}
												class="flex items-center gap-3 p-3 border border-hairline rounded-lg"
											>
												<span
													class={`badge ${isCorrect ? "bg-accent-emerald text-white" : "bg-accent-rose text-white"}`}
												>
													{isCorrect ? "✓" : "✗"}
												</span>
