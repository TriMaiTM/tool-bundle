...
export default function CompoundInterest() {
	...
	const calculation = useMemo(() => {
		...
		for (let year = 1; year <= Math.ceil(t); year++) {
			const balance = P * (1 + r / n) ** (n * year);
			const prevBalance = year === 1 ? P : P * (1 + r / n) ** (n * (year - 1));
		...
		<div class="space-y-6">
			...
			<div class="bg-surface-elevated rounded-lg p-6">
				<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
					<div>
						<label class="text-caption-uppercase text-muted block mb-2">Principal ($)</label>
						<input
							type="number"
							class="input w-full"
					...
					<div>
						<label class="text-caption-uppercase text-muted block mb-2">Compounding</label>
						<select
							class="input w-full"
							value={frequency}
							onChange={(e) =>
								setFrequency((e.target as HTMLSelectElement).value as CompoundingFrequency)
							}
						>
				...
				<div class="space-y-4">
					...
					<div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
						...
						<div class="bg-surface-elevated rounded-lg p-3 text-center">
							...
							<div class="text-caption text-muted mt-1">Total Interest Earned</div>
						</div>
						<div class="bg-surface-elevated rounded-lg p-3 text-center">
							<div class="text-title-lg text-primary">${formatCurrency(calculation.principal)}</div>
					...
					<div class="bg-surface-elevated rounded-lg p-6">
						<h3 class="text-title-sm text-body-strong mb-4">Growth Breakdown</h3>
						<div class="flex h-8 rounded-full overflow-hidden mb-3">
							<div class="bg-primary" style={`flex: ${calculation.principal}`} title="Principal" />
							<div
						...
						<div class="flex justify-between text-body-sm">
							<div class="flex items-center gap-2">
								...
								<span class="text-muted">
									...
									{((calculation.principal / calculation.finalBalance) * 100).toFixed(1)}
									%)
								</span>
							</div>
							<div class="flex items-center gap-2">
					...
					<div class="bg-surface-elevated rounded-lg p-6">
						<div class="flex items-center justify-between mb-4">
							<h3 class="text-title-sm text-body-strong">Year-by-Year Breakdown</h3>
							<button
								class="btn-secondary text-body-sm"
