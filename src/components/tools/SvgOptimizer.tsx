...
import { downloadText, readAsText, formatFileSize, copyToClipboard } from "../../utils/download";
...
function optimizeSvg(svg: string, options: OptimizeOptions): string {
  ...
  if (options.removeEditorNamespaces) {
    result = result.replace(/\s+xmlns:(?:sketch|inkscape|sodipodi|cc|dc|rdf|inkscape|bpmn|oai|serif|figma)="[^"]*"/gi, "");
...
export default function SvgOptimizer() {
  ...
  return (
    <div>
      {/* Input Mode Toggle */}
      <div class="flex rounded-md overflow-hidden border border-hairline mb-4" style="width: fit-content">
        <button
      ...
      {mode === "upload" ? (
        ...
        <div class="mb-6">
          <div class="flex items-center justify-between mb-2">
            ...
            <button class="text-body-sm text-primary hover:text-primary-active transition-colors" onClick={handleLoadSample}>
              Load Sample
            </button>
          </div>
          <textarea
          ...
          <div class="bg-surface-elevated rounded-lg p-3 mb-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
              {optionLabels.map(({ key, label }) => (
                <label key={key} class="flex items-center gap-2 text-body-sm text-body cursor-pointer">
                  <input
                    type="checkbox"
                    checked={options[key]}
