...
export default function BackgroundRemover() {
	...
	const handleProcess = useCallback(async () => {
		...
		try {
			...
			console.log("[BG Remover] mask object:", maskRaw, "keys:", Object.keys(maskRaw));
			...
			if (msg.includes("Unauthorized") || msg.includes("401")) {
				setError("Model access denied. Please try again or use a smaller image.");
				...
				<div>
					...
					<div class="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-6">
						<div>
							...
							<div class="text-caption text-muted mt-1">{formatFileSize(file.size)}</div>
						</div>
						<div>
							...
							{resultBlob && (
								<div class="text-caption text-muted mt-1">{formatFileSize(resultBlob.size)}</div>
							)}
						</div>
					</div>
