...
export default function ObjectDetection() {
	...
	const handleProcess = useCallback(async () => {
		...
		try {
			...
			setError(
				err instanceof Error ? err.message : "Failed to detect objects. Try a different image.",
			);
			setStatus("error");
		}
	}, [file]);
	...
	const handleCopyLabels = useCallback(async () => {
		...
		const text = detections.map((d) => `${d.label} (${(d.score * 100).toFixed(1)}%)`).join("\n");
		try {
				...
				<div>
					...
					<div class="relative mb-6" style="max-width: 640px">
						<div class="text-caption-uppercase text-muted mb-2">
							{detections.length > 0 ? `${detections.length} object(s) detected` : "Image Preview"}
						</div>
					...
					{status === "done" && detections.length > 0 && (
						<div class="mb-6">
							<div class="text-caption-uppercase text-muted mb-3">Detected Objects</div>
