...
export default function RandomNumberGenerator() {
  ...
  const generate = useCallback(() => {
    ...
    while (nums.length < actualCount && attempts < maxAttempts) {
      ...
      let num: number;
      if (integerOnly) {
        num = Math.floor(secureRandom() * (Math.floor(actualMax) - Math.ceil(actualMin) + 1)) + Math.ceil(actualMin);
      } else {
    ...
    setHistory((prev) => [
      {
        numbers: finalNums,
        timestamp: Date.now(),
        settings: { min: actualMin, max: actualMax, count: actualCount, allowDuplicates, integerOnly, sort: sortResults },
      },
      ...prev,
    ].slice(0, 5));
  ...
  return (
    <div>
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div>
          ...
          <input
            ...
            class="input w-full"
            value={min}
            onInput={(e) => setMin(parseFloat((e.target as HTMLInputElement).value) || 0)}
          />
        </div>
        <div>
          ...
          <input
            ...
            class="input w-full"
            value={max}
            onInput={(e) => setMax(parseFloat((e.target as HTMLInputElement).value) || 0)}
          />
        </div>
        <div>
          ...
          <input
            ...
            min={1}
            max={1000}
            onInput={(e) => setCount(Math.min(1000, Math.max(1, parseInt((e.target as HTMLInputElement).value) || 1)))}
          />
        </div>
      </div>
      ...
      <button class="btn-primary mb-6" onClick={generate}>Generate</button>
      ...
      {results.length > 0 && (
        <div class="mb-6">
          ...
          <div class="flex flex-wrap gap-2 mb-4">
            {results.map((num, i) => (
              <span class="badge-yellow" key={i}>{num}</span>
            ))}
          </div>
            ...
            <div class="bg-surface-elevated rounded-lg p-3">
              <div class="grid grid-cols-2 sm:grid-cols-5 gap-3">
                ...
                <div>
                  ...
                  <span class="text-body-strong">{integerOnly ? stats.sum : stats.sum.toFixed(2)}</span>
                </div>
                <div>
              ...
              <div class="bg-surface-elevated rounded-lg p-3" key={i}>
                ...
                <div class="flex flex-wrap gap-1">
                  {entry.numbers.map((num, j) => (
                    <span class="badge-yellow" key={j} style="font-size: 0.7rem;">{num}</span>
                  ))}
                </div>
              </div>
            ))}
      ...
      {!allowDuplicates && integerOnly && count > (Math.floor(Math.max(min, max)) - Math.ceil(Math.min(min, max)) + 1) && (
        <div class="bg-accent-rose/10 border border-accent-rose/30 rounded-lg p-4 mt-4 text-body-sm text-accent-rose">
          Cannot generate {count} unique integers in the range [{Math.min(min, max)}, {Math.max(min, max)}]. Increase the range or enable duplicates.
        </div>
      )}
    </div>
  );
