...
function tokenizeXml(xml: string): XmlToken[] {
  ...
  while (i < xml.length) {
    ...
    if (xml.startsWith("<!", i) && !xml.startsWith("<!--", i) && !xml.startsWith("<![CDATA[", i)) {
      const end = xml.indexOf(">", i);
      if (end !== -1) {
        tokens.push({ type: "doctype", content: xml.substring(i, end + 1), line });
        i = end + 1;
        continue;
      }
    }
    ...
    if (xml.startsWith("<?", i)) {
      const end = xml.indexOf("?>", i);
      if (end !== -1) {
        tokens.push({ type: "declaration", content: xml.substring(i, end + 2), line });
        i = end + 2;
        continue;
      }
    }
...
function validateXml(xml: string): ValidationResult {
  ...
  for (const token of tokens) {
    ...
    switch (token.type) {
      ...
      case "text":
        ...
        if (token.content.includes("&") && !token.content.match(/&(amp|lt|gt|quot|apos|#\d+|#x[0-9a-fA-F]+);/)) {
          return {
            valid: false,
  ...
  const hasDeclaration = xml.includes("<?xml");
          ...
          <>
            ...
            <label class="flex items-center gap-2 text-body-sm cursor-pointer">
              <input
                ...
                onChange={(e) =>
                  setSelfClosingTags((e.target as HTMLInputElement).checked)
                }
              />
              Self-closing tags
            </label>
          ...
          {validation.valid ? (
            ...
            <div>
              ...
              {validation.line !== null && (
                <span class="text-caption-uppercase text-muted ml-2">
