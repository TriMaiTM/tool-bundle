...
export default function TextTranslator() {
	...
	const loadModel = useCallback(async (modelId: string, onProgress: (p: number) => void) => {
	...
	const translateText = useCallback(async (translator: any, text: string, langTag?: string) => {
	...
	const handleTranslate = useCallback(async () => {
		...
		const _needsPivot = !isDirectToEn && !isDirectFromEn;
		...
		try {
			...
			if (isDirectFromEn) {
				...
				setStatusText(
					cached ? "Translating..." : "Loading English → Multi-language model (~300MB)...",
				);
				...
				finalText = await translateText(translator, input, LANG_TAGS[targetLang]);
			} else if (isDirectToEn) {
				// Any language → English (1 model)
				const cached = !!modelCache[MODEL_TO_EN];
				setStatusText(
					cached ? "Translating..." : "Loading Multi-language → English model (~300MB)...",
				);
				...
				finalText = await translateText(translatorFromEn, englishText, LANG_TAGS[targetLang]);
			}
	...
	return (
		<div>
			...
			<div class="flex flex-col sm:flex-row items-stretch sm:items-end gap-3 mb-6">
				{/* Source language */}
				<div class="flex-1">
					<label class="text-caption-uppercase text-muted block mb-1">From</label>
					<select
						class="input"
						value={sourceLang}
						onChange={(e) => {
			...
			{status !== "done" && (
				<div class="mb-4">
					<button
						...
						disabled={!input.trim() || isProcessing || sourceLang === targetLang}
					>
