...
import { hexToHsl, hslToHex, type Hsl } from "../../utils/color";
...
function generateSemanticColors(baseHsl: Hsl): { success: ColorShade[]; warning: ColorShade[]; error: ColorShade[]; info: ColorShade[] } {
  const semanticHues = {
    success: 142,  // green
    warning: 38,   // amber
    error: 0,      // rose
  ...
  const lightnessMap: Record<string, number> = {
    ...
    "500": 50, "600": 40, "700": 32, "800": 24, "900": 17,
  };
...
function generateCssVariables(primary: ColorShade[], neutral: ColorShade[], semantic: ReturnType<typeof generateSemanticColors>): string {
  const allColors = [...primary, ...neutral, ...semantic.success, ...semantic.warning, ...semantic.error, ...semantic.info];
  const vars = allColors.map((c) => `  --color-${c.name}: ${c.hex};`).join("\n");
  return `:root {\n${vars}\n}`;
}

function generateTailwindConfig(primary: ColorShade[], neutral: ColorShade[], semantic: ReturnType<typeof generateSemanticColors>): string {
  const formatGroup = (shades: ColorShade[], name: string) => {
    const entries = shades.map((c) => {
...
function generateScssVariables(primary: ColorShade[], neutral: ColorShade[], semantic: ReturnType<typeof generateSemanticColors>): string {
  const allColors = [...primary, ...neutral, ...semantic.success, ...semantic.warning, ...semantic.error, ...semantic.info];
...
export default function ColorSystemGenerator() {
  ...
  const cssOutput = useMemo(() => generateCssVariables(primary, neutral, semantic), [primary, neutral, semantic]);
  const tailwindOutput = useMemo(() => generateTailwindConfig(primary, neutral, semantic), [primary, neutral, semantic]);
  const scssOutput = useMemo(() => generateScssVariables(primary, neutral, semantic), [primary, neutral, semantic]);

  const activeOutput = useMemo(() => {
    switch (activeFormat) {
      case "css": return cssOutput;
      case "tailwind": return tailwindOutput;
      case "scss": return scssOutput;
    }
          ...
          {primary.map((shade) => (
            <div class="flex flex-col items-center gap-1">
              <div
                ...
                style={{ width: "56px", height: "40px", backgroundColor: shade.hex }}
                title={shade.hex}
              />
              <code class="text-caption-uppercase text-muted" style="font-family: var(--font-mono); font-size: 10px">
                {shade.name.split("-")[1]}
              </code>
              <code class="text-caption-uppercase text-muted" style="font-family: var(--font-mono); font-size: 9px">
                {shade.hex}
              </code>
            </div>
          ))}
              ...
              <div class="flex flex-wrap gap-1">
                {semantic[key].map((shade) => (
                  <div
                    ...
                    style={{ width: "32px", height: "24px", backgroundColor: shade.hex }}
