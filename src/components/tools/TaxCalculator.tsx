...
export default function TaxCalculator() {
	...
	const calculation = useMemo(() => {
		...
		if (Number.isNaN(grossIncome) || grossIncome < 0) return null;
		...
		<div class="space-y-6">
			...
			<div class="bg-surface-elevated rounded-lg p-6">
				<div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
					...
					<div>
						<label class="text-caption-uppercase text-muted block mb-2">Filing Status</label>
						<select
							class="input w-full"
							value={status}
							onChange={(e) => setStatus((e.target as HTMLSelectElement).value as FilingStatus)}
						>
			...
			{calculation && (
				<div class="space-y-4">
					...
					<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
						...
						<div class="bg-surface-elevated rounded-lg p-3 text-center">
							<div class="text-title-lg text-primary">{calculation.effectiveRate.toFixed(2)}%</div>
							<div class="text-caption text-muted mt-1">Effective Tax Rate</div>
						</div>
						<div class="bg-surface-elevated rounded-lg p-3 text-center">
							<div class="text-title-lg text-primary">{calculation.marginalRate.toFixed(0)}%</div>
					...
					<div class="bg-surface-elevated rounded-lg p-6">
						<h3 class="text-title-sm text-body-strong mb-4">Income Breakdown</h3>
						<div class="flex h-8 rounded-full overflow-hidden mb-3">
							<div
								...
								title="Take-Home Pay"
							/>
							<div class="bg-accent-rose" style={`flex: ${calculation.totalTax}`} title="Tax" />
							<div
					...
					<div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
						<div class="bg-surface-elevated rounded-lg p-3">
							<div class="text-caption-uppercase text-muted mb-1">Gross Income</div>
							<div class="text-body text-primary">${formatCurrency(calculation.grossIncome)}</div>
						</div>
						<div class="bg-surface-elevated rounded-lg p-3">
							<div class="text-caption-uppercase text-muted mb-1">
								Standard Deduction ({STATUS_LABELS[status]})
							</div>
							<div class="text-body text-primary">−${formatCurrency(calculation.deduction)}</div>
						</div>
						<div class="bg-surface-elevated rounded-lg p-3">
							<div class="text-caption-uppercase text-muted mb-1">Taxable Income</div>
							<div class="text-body text-primary">${formatCurrency(calculation.taxableIncome)}</div>
						</div>
						<div class="bg-surface-elevated rounded-lg p-3">
							<div class="text-caption-uppercase text-muted mb-1">Filing Status</div>
					...
					<div class="bg-surface-elevated rounded-lg p-6">
						<h3 class="text-title-sm text-body-strong mb-4">Tax Bracket Breakdown (2024)</h3>
						<div class="space-y-3">
							{calculation.bracketDetails.map((detail) => {
								const pctLabel = (detail.rate * 100).toFixed(0);
								const maxLabel =
									detail.bracketMax === Number.POSITIVE_INFINITY
										? "+"
										: `$${detail.bracketMax.toLocaleString()}`;
								return (
									<div key={detail.rate} class="flex items-center gap-3">
										<div class="w-20 shrink-0">
											<span class={`badge ${detail.taxableInBracket > 0 ? "badge-yellow" : ""}`}>
												{pctLabel}%
											</span>
										</div>
										<div class="flex-1">
							...
							<table class="w-full text-body-sm">
								<thead>
									<tr class="border-b border-hairline">
										<th class="text-left text-caption-uppercase text-muted py-2 px-3">Rate</th>
										...
										<th class="text-right text-caption-uppercase text-muted py-2 px-3">Tax</th>
									</tr>
								</thead>
								<tbody>
									{calculation.bracketDetails.map((detail) => {
										...
										return (
											...
											>
												<td class="py-2 px-3 font-medium">{(detail.rate * 100).toFixed(0)}%</td>
