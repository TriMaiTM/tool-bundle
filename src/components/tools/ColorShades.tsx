...
function generateShades(hex: string, count = 10): string[] {
...
export default function ColorShades() {
	...
	const handleCopyAllCss = useCallback(async () => {
		const css = shades.map((shade, i) => `  --shade-${(i + 1) * 100}: ${shade};`).join("\n");
		const output = `:root {\n${css}\n}`;
	...
	const handleCopyAllArray = useCallback(async () => {
		const output = JSON.stringify(shades);
		await navigator.clipboard.writeText(output);
		setCopiedAll("array");
		setTimeout(() => setCopiedAll(null), 1500);
	}, [shades]);
		...
		<div class="space-y-6">
			...
			<div class="bg-surface-elevated rounded-lg p-6">
				<div class="flex items-center justify-between mb-4">
					<label class="text-caption-uppercase text-muted">10 Shades</label>
				...
				<div class="grid grid-cols-2 sm:grid-cols-5 gap-3">
					{shades.map((shade, i) => (
						...
						>
							<div class="rounded-md mb-2" style={{ height: "48px", backgroundColor: shade }} />
							<code class="text-xs text-body" style="font-family: var(--font-mono)">
								{shade}
							</code>
							<div class="text-caption text-muted mt-1">
								{copiedIndex === i ? "Copied!" : `Shade ${(i + 1) * 100}`}
							</div>
						</button>
					))}
				</div>
