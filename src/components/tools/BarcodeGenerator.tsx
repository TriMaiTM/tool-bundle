...
export default function BarcodeGenerator() {
  ...
  const handleCopy = useCallback(async () => {
    ...
    const svgData = new XMLSerializer().serializeToString(svgRef.current);
    const blob = new Blob([svgData], { type: "image/svg+xml" });
    try { await navigator.clipboard.write([new ClipboardItem({ "image/svg+xml": blob })]); } catch { /* ignore */ }
  }, []);
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div>
          ...
          <div class="mb-4">
            ...
            <input class="input font-mono" value={text} onInput={(e) => setText((e.target as HTMLInputElement).value)} placeholder="Enter value..." />
          </div>

          {/* Format */}
          <div class="mb-4">
            <label class="text-caption-uppercase text-muted block mb-1">Format</label>
            <select class="input" value={format} onChange={(e) => setFormat((e.target as HTMLSelectElement).value)}>
          ...
          <div class="mb-4">
            ...
            <input type="range" min={1} max={5} value={width} onInput={(e) => setWidth(Number((e.target as HTMLInputElement).value))} class="w-full" />
          </div>

          {/* Height */}
          <div class="mb-4">
            <label class="text-caption-uppercase text-muted block mb-1">Height: {height}px</label>
            <input type="range" min={30} max={200} value={height} onInput={(e) => setHeight(Number((e.target as HTMLInputElement).value))} class="w-full" />
          </div>
          ...
          <div class="grid grid-cols-2 gap-4 mb-4">
            <div>
              ...
              <input type="color" value={lineColor} onInput={(e) => setLineColor((e.target as HTMLInputElement).value)} class="w-full h-10 rounded cursor-pointer border border-hairline" />
            </div>
            <div>
              <label class="text-caption-uppercase text-muted block mb-1">Background</label>
              <input type="color" value={bgColor} onInput={(e) => setBgColor((e.target as HTMLInputElement).value)} class="w-full h-10 rounded cursor-pointer border border-hairline" />
            </div>
          </div>

          {/* Download */}
          <div class="flex gap-3">
            <button class="btn-primary" onClick={handleDownload}>Download SVG</button>
            <button class="btn-secondary" onClick={handleCopy}>Copy SVG</button>
          </div>
        </div>
