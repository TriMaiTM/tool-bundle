...
const MAJOR_KEYWORDS = new Set([
  ...
  "SET", "DELETE", "CREATE", "ALTER", "DROP", "TABLE",
  "INDEX", "VIEW", "UNION", "UNION ALL", "EXCEPT", "INTERSECT",
  "CASE", "WHEN", "THEN", "ELSE", "END", "AS",
  "WITH", "DISTINCT", "TOP", "INTO", "FROM",
]);

const INDENT_KEYWORDS = new Set([
  "SELECT", "WHERE", "SET", "VALUES", "HAVING",
]);
...
function matchMultiWordKeyword(tokens: string[], pos: number): { keyword: string; length: number } | null {
...
function uppercaseKeywords(sql: string): string {
  const keywords = [
    ...
    "FULL", "CROSS", "ON", "AND", "OR", "ORDER", "BY", "GROUP", "HAVING",
    "LIMIT", "OFFSET", "INSERT", "INTO", "VALUES", "UPDATE", "SET",
    "DELETE", "CREATE", "ALTER", "DROP", "TABLE", "INDEX", "VIEW",
    "UNION", "EXCEPT", "INTERSECT", "AS", "DISTINCT", "TOP", "ALL",
    "CASE", "WHEN", "THEN", "ELSE", "END", "WITH", "NULL", "NOT",
    "IS", "IN", "LIKE", "BETWEEN", "EXISTS", "TRUE", "FALSE",
  ];
        ...
        {mode === "format" && (
          <>
            <select
              ...
              value={indentSize}
              onChange={(e) =>
                setIndentSize(Number((e.target as HTMLSelectElement).value))
              }
            >
            ...
            <label class="flex items-center gap-2 text-body-sm cursor-pointer">
              <input
                ...
                checked={uppercase}
                onChange={(e) =>
                  setUppercase((e.target as HTMLInputElement).checked)
                }
              />
              Uppercase keywords
            </label>
