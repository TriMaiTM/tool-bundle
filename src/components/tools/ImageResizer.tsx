...
import { type ImageFormat, loadImage, resizeImage } from "../../utils/image";
...
export default function ImageResizer() {
	...
	const handleResize = useCallback(async () => {
		...
		setProcessing(true);
		try {
			const finalW = mode === "pixels" ? width : Math.round((originalWidth * percentage) / 100);
			const finalH = mode === "pixels" ? height : Math.round((originalHeight * percentage) / 100);
			const blob = await resizeImage(file, finalW, finalH, format, quality / 100);
	...
	}, [file, width, height, percentage, mode, format, quality, originalWidth, originalHeight]);
		...
		<div>
			{!file ? (
				...
				<div>
					...
					<div class="flex flex-col sm:flex-row gap-6 mb-6">
						<div class="flex-shrink-0">
							...
							<div class="text-caption text-muted mt-2 text-center">
								Original: {originalWidth} × {originalHeight} ({formatFileSize(file.size)})
							</div>
						</div>
						<div class="flex-1">
							{/* Mode toggle */}
							...
							{mode === "pixels" ? (
									...
									<div class="flex items-center gap-3">
										<div class="flex-1">
											...
											<input
												...
												value={width}
												onInput={(e) =>
													handleWidthChange(Number((e.target as HTMLInputElement).value) || 0)
												}
											/>
										</div>
										<div class="pt-6">
										...
										<div class="flex-1">
											...
											<input
												...
												value={height}
												onInput={(e) =>
													handleHeightChange(Number((e.target as HTMLInputElement).value) || 0)
												}
											/>
										</div>
									</div>
								...
								<div>
									...
									<input
										...
										value={percentage}
										onInput={(e) =>
											handlePercentageChange(Number((e.target as HTMLInputElement).value))
										}
										class="w-full"
					...
					{/* Presets */}
					<div class="mb-6">
						<label class="text-caption-uppercase text-muted block mb-2">Social Media Presets</label>
					...
					<div class="flex flex-wrap gap-3">
						<button class="btn-primary" onClick={handleResize} disabled={processing}>
