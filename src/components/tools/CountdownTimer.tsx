	...
	const addCountdown = useCallback(
		(target: Date, label?: string) => {
			setCountdowns((prev) => [
				...
				{
					id: nextId++,
					label: label || labelInput || `Countdown ${prev.length + 1}`,
					target,
				},
			]);
			setDateInput("");
						...
						<div class="bg-surface-elevated rounded-lg p-6">
							<div class="flex items-center justify-between mb-4">
								<div>
									...
									<div class="text-caption text-muted">Target: {cd.target.toLocaleString()}</div>
								</div>
								<button class="btn-secondary text-sm" onClick={() => removeCountdown(cd.id)}>
									Remove
								</button>
							</div>
							...
							{remaining.expired ? (
								<div class="text-center py-4">
									<div
										class="text-display-sm text-primary"
