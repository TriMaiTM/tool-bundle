...
function calculateInvestment(
  ...
  annualRate: number,
  years: number
): { finalValue: number; totalContributions: number; totalEarnings: number; breakdown: YearRow[] } {
  ...
  for (let month = 1; month <= totalMonths; month++) {
    ...
    if (month % 12 === 0) {
      ...
      const prevBalance = year === 1
        ? initial * (1 + r) + monthly * ((Math.pow(1 + r, 12) - 1) / r) * (1 + r)
        : breakdown[breakdown.length - 2]?.balance
          ? (breakdown[breakdown.length - 2].balance + monthly) * (1 + r)
          : initial;
      const yearEarnings = balance - prevContributions - (initial + (month - 12) * monthly > 0 ? 0 : 0);
...
export default function InvestmentCalculator() {
  ...
  const scenarios = useMemo<Scenario[]>(() => {
    ...
    if (isNaN(P) || isNaN(PMT) || isNaN(t) || P < 0 || PMT < 0 || t <= 0) {
      return [];
    }
    ...
    <div class="space-y-6">
      ...
      <div class="bg-surface-elevated rounded-lg p-6">
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <div>
            <label class="text-caption-uppercase text-muted block mb-2">Initial Investment ($)</label>
            <input
              type="number"
              class="input w-full"
          ...
          <div>
            <label class="text-caption-uppercase text-muted block mb-2">Monthly Contribution ($)</label>
            <input
              type="number"
              class="input w-full"
              placeholder="e.g. 500"
        ...
        <div class="space-y-4">
          <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <div class="bg-surface-elevated rounded-lg p-3 text-center">
              <div class="text-title-lg text-primary">${formatCurrency(primaryCalc.finalValue)}</div>
              <div class="text-caption text-muted mt-1">Final Value</div>
            </div>
            <div class="bg-surface-elevated rounded-lg p-3 text-center">
              <div class="text-title-lg text-primary">${formatCurrency(primaryCalc.totalContributions)}</div>
              <div class="text-caption text-muted mt-1">Total Contributions</div>
            </div>
            <div class="bg-surface-elevated rounded-lg p-3 text-center">
              <div class="text-title-lg text-accent-emerald">${formatCurrency(primaryCalc.totalEarnings)}</div>
            ...
            <div class="flex justify-between text-body-sm">
              <div class="flex items-center gap-2">
                ...
                <span class="text-muted">
                  Contributions: ${formatCurrency(primaryCalc.totalContributions)} ({((primaryCalc.totalContributions / primaryCalc.finalValue) * 100).toFixed(1)}%)
                </span>
              </div>
              <div class="flex items-center gap-2">
                <div class="w-3 h-3 rounded-full bg-accent-emerald" />
                <span class="text-muted">
                  Earnings: ${formatCurrency(primaryCalc.totalEarnings)} ({((primaryCalc.totalEarnings / primaryCalc.finalValue) * 100).toFixed(1)}%)
                </span>
              </div>
            </div>
          </div>
              ...
              <table class="w-full text-body-sm">
                <thead>
                  <tr class="border-b border-hairline">
                    ...
                    <th class="text-right text-caption-uppercase text-muted py-2 px-3">Contributions</th>
                ...
                <tbody>
                  {primaryCalc.breakdown.map((row) => (
                    <tr key={row.year} class="border-b border-hairline/50 hover:bg-surface-soft/50 transition-colors">
                      <td class="py-2 px-3 text-muted">{row.year}</td>
                      <td class="py-2 px-3 text-right text-primary">${formatCurrency(row.contributions)}</td>
                      <td class="py-2 px-3 text-right text-accent-emerald">${formatCurrency(row.earnings)}</td>
                      <td class="py-2 px-3 text-right font-medium text-primary">${formatCurrency(row.balance)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
