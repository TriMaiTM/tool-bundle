...
export default function RobotsTxtGenerator() {
  ...
  const addAgent = useCallback(() => {
    ...
    setState((s) => ({
      ...
      additionalAgents: [
        ...s.additionalAgents,
        { agent: newAgentName.trim(), rules: [{ type: "Disallow" as const, path: "/private" }] },
      ],
    }));
    setNewAgentName("");
  }, [newAgentName]);
  ...
  const addAgentRule = useCallback((agentIndex: number) => {
    setState((s) => ({
      ...
      additionalAgents: s.additionalAgents.map((a, i) =>
        i === agentIndex ? { ...a, rules: [...a.rules, { type: "Disallow" as const, path: "/" }] } : a
      ),
    }));
  }, []);
    ...
    <div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div>
          ...
          <div class="bg-surface-elevated rounded-lg p-3 mb-4">
            ...
            <div class="flex items-center gap-3 mb-3">
              ...
              <input
                ...
                onInput={(e) =>
                  setState((s) => ({ ...s, defaultAgent: (e.target as HTMLInputElement).value }))
                }
              />
            </div>
            ...
            <div class="flex items-center gap-2">
              <select
                ...
                value={newRuleType}
                onChange={(e) => setNewRuleType((e.target as HTMLSelectElement).value as "Allow" | "Disallow")}
              >
              ...
              />
              <button class="btn-primary text-body-sm" style="height: 36px; padding: 0 14px" onClick={addRule}>
                Add
              </button>
            </div>
          </div>
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-4">
            <div>
              ...
              <input
                ...
                value={state.sitemap}
                onInput={(e) => setState((s) => ({ ...s, sitemap: (e.target as HTMLInputElement).value }))}
              />
            </div>
            <div>
              <label class="text-caption-uppercase text-muted block mb-2">Crawl Delay (seconds)</label>
              <input
                class="input"
                type="number"
                ...
                onInput={(e) => setState((s) => ({ ...s, crawlDelay: (e.target as HTMLInputElement).value }))}
              />
            </div>
          </div>
          ...
          <div class="bg-surface-elevated rounded-lg p-3 mb-4">
            <label class="text-caption-uppercase text-muted block mb-3">Additional User Agents</label>

            {state.additionalAgents.map((agent, ai) => (
              <div
                key={ai}
                class="mb-4 pb-4"
                style={ai < state.additionalAgents.length - 1 ? "border-bottom: 1px solid var(--color-hairline)" : ""}
              >
            ...
            <div class="flex items-center gap-2">
              ...
              />
              <button class="btn-primary text-body-sm" style="height: 36px; padding: 0 14px" onClick={addAgent}>
                Add Agent
              </button>
            </div>
          </div>
