...
const TIMEZONES: TimezoneInfo[] = [
	...
	{
		id: "Australia/Sydney",
		label: "AEST / AEDT (Sydney)",
		region: "Australia",
	},
	{
		id: "Pacific/Auckland",
		label: "NZST / NZDT (Auckland)",
		region: "Pacific",
	},
	{
...
export default function TimezoneConverter() {
	...
	const convertedTime = useMemo(() => {
		try {
			...
			const _sourceFormatted = new Intl.DateTimeFormat("en-US", {
				timeZone: sourceTz,
				year: "numeric",
				month: "2-digit",
				day: "2-digit",
				...
				<div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
					<div>
						...
						>
							{TIMEZONES.map((tz) => (
								<option value={tz.id}>
									{tz.label} ({getOffsetString(tz.id)})
								</option>
							))}
						</select>
					</div>
				...
				<div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
					<div class="bg-surface-elevated rounded-lg p-6 text-center">
						...
						<div class="text-caption text-primary mb-2">
							{sourceTz} · {convertedTime.sourceOffset}
						</div>
					...
					<div class="bg-surface-elevated rounded-lg p-6 text-center">
						...
						<div class="text-caption text-primary mb-2">
							{targetTz} · {convertedTime.targetOffset}
						</div>
						...
						<tbody>
							{TIMEZONES.map((tz) => (
								<tr class="border-b border-hairline">
									...
									<td class="p-2" style="font-family: var(--font-mono)">
										{formatTime(new Date(), tz.id)}
									</td>
								</tr>
							))}
						</tbody>
