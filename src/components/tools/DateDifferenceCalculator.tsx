...
export default function DateDifferenceCalculator() {
  ...
  const endDayOfWeek = useMemo(() => {
    ...
    return new Date(endDate + "T00:00:00").toLocaleDateString("en-US", { weekday: "long" });
  }, [endDate]);
      ...
      <div class="bg-surface-elevated rounded-lg p-6">
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
          <div>
            ...
            />
            {startDate && (
              <div class="text-caption text-primary mt-1">{startDayOfWeek} · {startLeap ? "Leap year" : "Not a leap year"}</div>
            )}
          </div>
          <div>
            ...
            />
            {endDate && (
              <div class="text-caption text-primary mt-1">{endDayOfWeek} · {endLeap ? "Leap year" : "Not a leap year"}</div>
            )}
          </div>
        </div>
        ...
        <div class="space-y-4">
          ...
          <div class="bg-surface-elevated rounded-lg p-6 text-center">
            ...
            <div class="text-body-sm text-muted mt-2">
              {result.direction >= 0 ? "Start date is before end date" : "Start date is after end date"}
            </div>
          </div>
