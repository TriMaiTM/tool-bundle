...
export default function BreakEvenCalculator() {
	...
	const calculation = useMemo(() => {
		const fc = Number.parseFloat(fixedCosts);
		const vc = Number.parseFloat(variableCost);
		const sp = Number.parseFloat(sellingPrice);

		if (Number.isNaN(fc) || Number.isNaN(vc) || Number.isNaN(sp) || fc < 0 || vc < 0 || sp <= 0)
			return null;
	...
	const profitCalc = useMemo(() => {
		...
		const units = Number.parseInt(sellUnits);
		if (Number.isNaN(units) || units < 0) return null;
	...
	return (
		<div class="space-y-6">
			...
			<div class="bg-surface-elevated rounded-lg p-6">
				...
				{Number.parseFloat(sellingPrice) > 0 &&
					Number.parseFloat(variableCost) > 0 &&
					Number.parseFloat(sellingPrice) <= Number.parseFloat(variableCost) && (
					...
					<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
						<div class="bg-surface-elevated rounded-lg p-3 text-center">
							<div class="text-title-lg text-primary">
								{calculation.breakEvenUnits.toLocaleString()}
							</div>
						...
						<div class="bg-surface-elevated rounded-lg p-3 text-center">
							<div class="text-title-lg text-accent-emerald">
								{(calculation.contributionMarginRatio * 100).toFixed(1)}%
							</div>
							...
							{profitCalc && (
								<div class="space-y-2">
									...
									<div class="flex justify-between text-body font-medium border-t border-hairline pt-2 mt-2">
										<span>Profit</span>
										<span
											class={profitCalc.profit >= 0 ? "text-accent-emerald" : "text-accent-rose"}
										>
											{profitCalc.profit < 0 ? "−" : ""}$
											{formatCurrency(Math.abs(profitCalc.profit))}
										</span>
									</div>
								</div>
							)}
										...
										{scenarios.map((s) => (
											...
											>
												...
												<td class="py-2 px-3 text-right text-primary font-medium">
													{s.breakEvenUnits.toLocaleString()}
												</td>
												<td class="py-2 px-3 text-right text-muted">
													${formatCurrency(s.breakEvenRevenue)}
												</td>
											</tr>
										))}
									</tbody>
