		...
		<div>
			<div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
				<div>
					...
					<select
						class="input w-full"
						value={diceType}
						onChange={(e) => setDiceType(Number.parseInt((e.target as HTMLSelectElement).value))}
					>
						{DICE_TYPES.map((d) => (
				...
				<div>
					...
					<input
						...
						class="input w-full"
						value={modifier}
						onInput={(e) => setModifier(Number.parseInt((e.target as HTMLInputElement).value) || 0)}
					/>
				</div>
			</div>
			...
			{rolling && (
				<div class="bg-surface-elevated rounded-lg p-3 mb-6 text-center">
					<span class="text-title-lg text-primary" style="font-size: 2rem;">
						🎲 ?
					</span>
				</div>
			)}
						...
						<div class="flex flex-wrap items-center gap-3 mb-3">
							{results.map((r, i) => (
								<div class="text-center" key={i}>
									...
									<div class="text-body-sm text-muted-soft mt-1">
										D{diceType} #{i + 1}
									</div>
								</div>
							))}
					...
					<div class="space-y-2">
						{history.map((roll) => (
							<div class="bg-surface-elevated rounded-lg p-3" key={roll.id}>
								...
								<span class="text-body-sm text-muted-soft">[{roll.results.join(", ")}]</span>
							</div>
						))}
					</div>
				</div>
