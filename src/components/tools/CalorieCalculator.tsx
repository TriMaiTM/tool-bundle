...
const ACTIVITY_LEVELS: ActivityOption[] = [
	...
	{
		value: "extra",
		label: "Extra Active (physical job + exercise)",
		multiplier: 1.9,
	},
];
...
export default function CalorieCalculator() {
	...
	const result = useMemo(() => {
		...
		if (Number.isNaN(a) || a <= 0 || Number.isNaN(w) || w <= 0 || hCm === null || hCm <= 0) {
			return null;
		}
		...
		// Mifflin-St Jeor
		const bmr =
			gender === "male" ? 10 * wKg + 6.25 * hCm - 5 * a + 5 : 10 * wKg + 6.25 * hCm - 5 * a - 161;
				...
				<div class="space-y-4">
					...
					<div class="bg-surface-elevated rounded-lg p-6">
						<h3 class="text-title-sm text-body-strong mb-4">
							Macronutrient Breakdown (at maintenance)
						</h3>

						{/* Visual bar */}
						<div class="flex h-6 rounded-full overflow-hidden mb-4">
							<div class="bg-accent-blue" style="flex: 30" title="Protein 30%" />
							<div class="bg-warning" style="flex: 25" title="Fat 25%" />
							<div class="bg-accent-emerald" style="flex: 45" title="Carbs 45%" />
						</div>

						<div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
							<div class="p-3 rounded-lg bg-surface-soft">
								<div class="flex items-center gap-2 mb-2">
									<div class="w-3 h-3 rounded-full bg-accent-blue" />
