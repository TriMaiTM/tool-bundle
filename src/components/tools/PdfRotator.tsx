...
export default function PdfRotator() {
	...
	const handleRotate = useCallback(async () => {
		...
		try {
			...
			const pagesToRotate =
				selectMode === "all" ? pages.map((_, i) => i + 1) : Array.from(selectedPages);
		...
		<div>
			{!file ? (
				...
				<div>
					...
					{/* Rotation Angle */}
					<div class="mb-6">
						<label class="text-caption-uppercase text-muted block mb-2">Rotation Angle</label>
					...
					{/* Page Selection */}
					<div class="mb-6">
						<label class="text-caption-uppercase text-muted block mb-2">Pages to Rotate</label>
					...
					<div class="bg-accent-emerald/10 border border-accent-emerald/30 rounded-lg p-4 mb-6">
						<div class="text-body-sm text-body">
							Will rotate{" "}
							<strong>
								{pagesToRotateCount} page{pagesToRotateCount !== 1 ? "s" : ""}
							</strong>{" "}
							by <strong>{rotationAngle}°</strong> clockwise
						</div>
					</div>
