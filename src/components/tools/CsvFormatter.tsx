	...
	return fields
		.map((f) => {
			...
			if (needsQuote) {
				return (
					quoteChar +
					f.replace(new RegExp(quoteChar === '"' ? '"' : "'", "g"), quoteChar + quoteChar) +
					quoteChar
				);
			}
			return f;
...
export default function CsvFormatter() {
	...
	const validation = useMemo(() => {
		if (parsedRows.length === 0)
			return {
				valid: true,
				errors: [] as { line: number; expected: number; got: number }[],
			};
			...
			<div class="flex flex-wrap items-center gap-3 mb-6">
				<div class="flex items-center gap-2">
					...
					<select
						...
						onChange={(e) =>
							setDelimiterSetting((e.target as HTMLSelectElement).value as Delimiter | "auto")
						}
					>
						{DELIMITERS.map((d) => (
			...
			{parsedRows.length > 0 && (
				<div class="mb-4">
					<div class="flex items-center justify-between mb-2">
						...
						{hasMoreRows && (
							<span class="text-caption text-muted">Showing 100 of {parsedRows.length} rows</span>
						)}
					</div>
					<div class="bg-surface-elevated rounded-lg" style="max-height: 360px; overflow: auto">
