...
export default function JwtDecoder() {
  ...
  const handleDecode = useCallback(() => {
    if (!input.trim()) { setToken(null); setError(null); return; }
    try {
      setError(null);
    ...
    <div>
      ...
      <div class="mb-4">
        ...
        <textarea
          ...
          value={input}
          onInput={(e) => { setInput((e.target as HTMLTextAreaElement).value); if (token) { setToken(null); } }}
        />
      </div>

      {/* Decode button */}
      <div class="mb-4">
        <button class="btn-primary" onClick={handleDecode} disabled={!input.trim()}>Decode JWT</button>
      </div>
        ...
        <div class="space-y-4">
          ...
          <div class="card">
            <div class="flex items-center justify-between mb-2">
              ...
              <button class="text-body-sm text-primary hover:text-primary-active" onClick={() => handleCopy(JSON.stringify(token.header, null, 2), "header")}>
                {copied === "header" ? "Copied!" : "Copy"}
              </button>
            </div>
            <pre class="code-block font-mono" style="font-size: 13px"><code>{JSON.stringify(token.header, null, 2)}</code></pre>
          </div>
          ...
          <div class="card">
            <div class="flex items-center justify-between mb-2">
              ...
              <button class="text-body-sm text-primary hover:text-primary-active" onClick={() => handleCopy(JSON.stringify(token.payload, null, 2), "payload")}>
                {copied === "payload" ? "Copied!" : "Copy"}
              </button>
            </div>
            <pre class="code-block font-mono" style="font-size: 13px"><code>{JSON.stringify(token.payload, null, 2)}</code></pre>
            ...
            <div class="mt-3 space-y-2">
              {token.payload.exp && (
                <div class="flex items-center gap-2">
                  ...
                  <span class={`text-body-sm ${isExpired(token.payload.exp) ? "text-accent-rose" : "text-accent-emerald"}`}>
          ...
          <div class="card">
            <div class="flex items-center justify-between mb-2">
              ...
              <button class="text-body-sm text-primary hover:text-primary-active" onClick={() => handleCopy(token.signature, "sig")}>
                {copied === "sig" ? "Copied!" : "Copy"}
              </button>
            </div>
            <pre class="code-block font-mono text-body-sm" style="font-size: 12px; word-break: break-all"><code>{token.signature}</code></pre>
          </div>
        </div>
      )}
    </div>
