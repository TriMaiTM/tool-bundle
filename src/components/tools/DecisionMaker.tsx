    ...
    if (mode === "random" || mode === "yesnomaybe") {
      ...
      intervalRef.current = setInterval(() => {
        ...
        if (tick >= 20) {
          ...
          setDeciding(false);
          playRevealSound();
          setHistory((prev) => [{ result: chosen, mode, timestamp: Date.now() }, ...prev].slice(0, 20));
        }
      }, 80);
    } else if (mode === "wheel") {
      const arr = new Uint32Array(1);
      ...
      const animate = () => {
        ...
        if (progress < 1) {
          ...
          setDeciding(false);
          playRevealSound();
          setHistory((prev) => [{ result: selected, mode, timestamp: Date.now() }, ...prev].slice(0, 20));
        }
      };
      requestAnimationFrame(animate);
      ...
      const eliminateNext = () => {
        if (available.length <= 1) {
          ...
          setDeciding(false);
          playRevealSound();
          setHistory((prev) => [{ result: available[0], mode, timestamp: Date.now() }, ...prev].slice(0, 20));
          return;
        }
        const arr = new Uint32Array(1);
        crypto.getRandomValues(arr);
  ...
  return (
    <div>
        ...
        <div class="flex flex-wrap gap-2">
          {modes.map((m) => (
            <button
              ...
              onClick={() => { setMode(m.key); setResult(null); setEliminated([]); }}
            >
              {m.label}
            </button>
          ))}
      ...
      <button
        ...
        onClick={decide}
        disabled={deciding || options.length < 2}
      >
      ...
      {mode === "elimination" && eliminated.length > 0 && deciding && (
        <div class="bg-surface-elevated rounded-lg p-4 mb-6">
          <span class="text-caption-uppercase text-muted block mb-2">Elimination Round {eliminationRound}</span>
          <div class="text-body-sm text-accent-rose mb-2">Eliminated: {eliminated[eliminated.length - 1]}</div>
          <div class="text-body-strong text-primary">Remaining: {options.filter((o) => !eliminated.includes(o)).join(", ")}</div>
        </div>
      )}
      ...
      {result && !deciding && (
        <div class="bg-surface-elevated rounded-lg p-6 mb-6 text-center">
          ...
          <div class="text-title-lg text-primary" style="font-size: 2.5rem;">{result}</div>
        </div>
      )}
              ...
              <div class="bg-surface-elevated rounded-lg p-3" key={i}>
                <div class="flex items-center justify-between">
                  <div class="flex items-center gap-2">
                    ...
                    <span class="text-body-sm text-muted-soft capitalize">via {entry.mode === "yesnomaybe" ? "Yes/No/Maybe" : entry.mode}</span>
                  </div>
