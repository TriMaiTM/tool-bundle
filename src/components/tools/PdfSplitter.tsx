	...
	const parsePageRanges = useCallback((rangeStr: string, max: number): number[] => {
		...
		for (const part of parts) {
			if (part.includes("-")) {
				...
				if (Number.isNaN(start) || Number.isNaN(end) || start < 1 || end < start || end > max) {
		...
		<div>
			{!file ? (
				...
				<div>
					...
					{/* Page Range Input */}
					<div class="mb-6">
						<label class="text-caption-uppercase text-muted block mb-2">Page Range</label>
						<input
							type="text"
							class="input"
						...
						<div class="text-caption text-muted mt-2">
							Enter page numbers or ranges separated by commas. Pages: 1-
							{totalPages}
						</div>
					</div>
					...
					{/* Preset Ranges */}
					<div class="mb-6">
						<label class="text-caption-uppercase text-muted block mb-2">Quick Select</label>
					...
					<div class="flex flex-wrap gap-3">
						<button class="btn-primary" onClick={handleSplit} disabled={processing || !pageRange}>
