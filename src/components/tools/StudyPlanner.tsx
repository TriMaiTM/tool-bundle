...
export default function StudyPlanner() {
	...
	const addExam = useCallback(() => {
		...
		const hrs = Number.parseFloat(hoursNeeded);
	...
	const stats = useMemo(() => {
		...
		const completed = sessions.filter((s) => s.completed).reduce((sum, s) => sum + s.hours, 0);
	...
	return (
		<div class="space-y-6">
			...
			<div class="bg-surface-elevated rounded-lg p-3">
				<h3 class="text-title-lg text-primary mb-4">{editId ? "Edit Exam" : "Add Exam"}</h3>
				<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
					<div>
						<label class="text-caption-uppercase text-muted block mb-2">Subject Name</label>
						<input
							type="text"
							class="input w-full"
					...
					<div>
						<label class="text-caption-uppercase text-muted block mb-2">Exam Date</label>
						<input
							type="date"
							class="input w-full"
							value={examDate}
					...
					<div>
						<label class="text-caption-uppercase text-muted block mb-2">Difficulty</label>
						<select
							class="input w-full"
							value={difficulty}
							onChange={(e) =>
								setDifficulty((e.target as HTMLSelectElement).value as "easy" | "medium" | "hard")
							}
						>
					...
					<div>
						<label class="text-caption-uppercase text-muted block mb-2">Hours Needed</label>
						<input
							type="number"
							class="input w-full"
							placeholder="e.g. 20"
							min={1}
							value={hoursNeeded}
							onInput={(e) => setHoursNeeded((e.target as HTMLInputElement).value)}
						/>
					</div>
				</div>
			...
			{sortedExams.length > 0 && (
				<div class="bg-surface-elevated rounded-lg p-3">
					<div class="flex items-center justify-between mb-4">
						<h3 class="text-title-lg text-primary">Exams ({sortedExams.length})</h3>
								...
								>
									<div class="flex-1 min-w-0 mr-3">
										...
										<div class="text-body-sm text-muted">
											{formatDate(exam.date)} · {days > 0 ? `${days} days away` : "Past due"} ·{" "}
											<span class={diffColor}>{exam.difficulty}</span> · {exam.hoursNeeded}h needed
										</div>
									</div>
									<div class="flex gap-2 shrink-0">
										<button class="btn-secondary text-body-sm" onClick={() => startEdit(exam)}>
											Edit
										</button>
										<button class="btn-secondary text-body-sm" onClick={() => deleteExam(exam.id)}>
											Delete
										</button>
									</div>
								</div>
			...
			{sessions.length > 0 && (
				<div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
					<div class="bg-surface-elevated rounded-lg p-3 text-center">
						<div class="text-title-lg text-primary">{stats.total.toFixed(1)}h</div>
						<div class="text-caption text-muted mt-1">Total Study Hours</div>
					</div>
					<div class="bg-surface-elevated rounded-lg p-3 text-center">
						<div class="text-title-lg text-accent-emerald">{stats.completed.toFixed(1)}h</div>
						<div class="text-caption text-muted mt-1">Hours Completed</div>
					</div>
					<div class="bg-surface-elevated rounded-lg p-3 text-center">
						<div class="text-title-lg text-accent-rose">{stats.remaining.toFixed(1)}h</div>
			...
			{sessions.length > 0 && (
				<div class="bg-surface-elevated rounded-lg p-3">
					...
					{/* Date Navigator */}
					<div class="flex items-center justify-between mb-4">
						<button class="btn-secondary text-body-sm" onClick={() => shiftDate(-1)}>
							&larr; Previous
						</button>
					...
					{daySessions.length > 0 ? (
							...
							{daySessions.map((session) => {
								...
								return (
									...
									>
										...
										<div class="flex-1">
											<span
												class={session.completed ? "line-through text-muted" : "text-body-strong"}
											>
												{exam?.subject || "Unknown"}
											</span>
											<span class="text-body-sm text-muted ml-2">{session.hours}h</span>
										</div>
										<span
											class={`badge ${
												exam?.difficulty === "hard"
						...
						<p class="text-muted text-body-sm text-center py-4">No study sessions for this date.</p>
					)}
				...
				<div class="bg-surface-elevated rounded-lg p-3">
					...
					<div class="grid grid-cols-7 gap-1">
						...
						{(() => {
							...
							const _lastDate = new Date(`${allDates[allDates.length - 1]}T00:00:00`);
							const totalDays = daysBetween(allDates[0], allDates[allDates.length - 1]) + 1;
							const cells: JSX.Element[] = [];

							// Pad with empty cells
							for (let i = 0; i < startDay; i++) {
								cells.push(<div key={`pad-${i}`} class="p-1" />);
							}
							...
							for (let i = 0; i < totalDays; i++) {
								...
								const completedCount = daySess.filter((s) => s.completed).length;
					...
					<div class="flex flex-wrap gap-3 mt-3 text-body-sm">
						<div class="flex items-center gap-2">
							<div class="w-3 h-3 rounded bg-primary/20" />
							<span class="text-muted">In progress</span>
						</div>
						<div class="flex items-center gap-2">
							<div class="w-3 h-3 rounded bg-accent-emerald/20" />
