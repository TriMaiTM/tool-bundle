			...
			<div class="flex flex-wrap items-end gap-4 mb-6">
				<div>
					...
					<input
						...
						onInput={(e) =>
							setCount(
								Math.min(100, Math.max(1, Number((e.target as HTMLInputElement).value) || 1)),
							)
						}
						style="width: 80px"
					/>
				...
				<div>
					...
					>
						{FORMATS.map((f) => (
							<button
								key={f.id}
								class={`px-3 py-2 text-body-sm font-medium transition-colors ${format === f.id ? "bg-primary text-on-primary" : "bg-surface-elevated text-body hover:text-on-dark"}`}
					...
					<div class="space-y-2">
						{uuids.map((uuid, i) => (
							<div key={i} class="flex items-center gap-3 bg-surface-elevated rounded-lg p-3">
								...
								<code class="font-mono text-body-sm text-on-dark flex-1">
