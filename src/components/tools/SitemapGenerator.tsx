...
function createEntry(url = ""): SitemapEntry {
  return { id: newId(), url, priority: "0.5", changefreq: "monthly", lastmod: "" };
}

const PRIORITIES = ["1.0", "0.9", "0.8", "0.7", "0.6", "0.5", "0.4", "0.3", "0.2", "0.1"];
const CHANGE_FREQS: ChangeFreq[] = ["always", "hourly", "daily", "weekly", "monthly", "yearly", "never"];
...
export default function SitemapGenerator() {
  ...
  const addEntry = useCallback(() => {
    ...
    setEntries((prev) => [
      ...prev,
      { id: newId(), url: newUrl.trim(), priority: newPriority, changefreq: newChangefreq, lastmod: newLastmod },
    ]);
    setNewUrl("");
    setNewLastmod("");
        ...
        <div>
          {mode === "manual" ? (
            <div class="bg-surface-elevated rounded-lg p-3 mb-4">
                ...
                <div class="grid grid-cols-3 gap-2">
                  ...
                  <div>
                    ...
                    <select
                      class="input"
                      value={newChangefreq}
                      onChange={(e) => setNewChangefreq((e.target as HTMLSelectElement).value as ChangeFreq)}
                    >
                      {CHANGE_FREQS.map((f) => (
              ...
              <button class="btn-primary text-body-sm" style="height: 36px; padding: 0 16px" onClick={addEntry}>
                Add URL
              </button>
            </div>
          ) : (
            <div class="mb-4">
              <label class="text-caption-uppercase text-muted block mb-2">Paste URLs (one per line)</label>
              <textarea
                class="textarea"
              ...
              />
              <button class="btn-primary text-body-sm mt-3" style="height: 36px; padding: 0 16px" onClick={parseBulk}>
                Parse URLs
              </button>
            </div>
          )}
                ...
                {entries.map((entry) => (
                  ...
                  >
                    <div class="flex-1 min-w-0">
                      <div class="text-body-sm truncate" style="font-family: var(--font-mono); font-size: 12px">
                        {entry.url}
                      </div>
                      <div class="flex items-center gap-3 mt-1">
                        ...
                        {entry.lastmod && <span class="text-caption text-muted">Mod: {entry.lastmod}</span>}
                      </div>
                    </div>
                    <button
