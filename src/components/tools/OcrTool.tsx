...
export default function OcrTool() {
	...
	const handleProcess = useCallback(async () => {
		...
		try {
			...
			const { text, confidence: conf } = await performOCR(file, lang, (p, s) => {
				if (!abortRef.current) {
					setProgress(p);
					setStatusText(s);
				}
						...
						<div class="mb-6">
							<div class="flex items-center justify-between mb-2">
								<span class="text-body-sm text-body">
									{status === "loading-engine" ? "Loading OCR engine..." : "Extracting text..."}
								</span>
							...
							{statusText && <p class="text-caption text-muted mt-2">{statusText}</p>}
