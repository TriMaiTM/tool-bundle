...
export default function MetaTagGenerator() {
	...
	const standardTags = useMemo(() => {
		...
		if (state.description)
			lines.push(`<meta name="description" content="${escapeHtml(state.description)}" />`);
		if (state.keywords)
			lines.push(`<meta name="keywords" content="${escapeHtml(state.keywords)}" />`);
	...
	const ogTags = useMemo(() => {
		const lines: string[] = [];
		if (state.title)
			lines.push(`<meta property="og:title" content="${escapeHtml(state.title)}" />`);
		if (state.description)
			lines.push(`<meta property="og:description" content="${escapeHtml(state.description)}" />`);
		if (state.ogImage)
			lines.push(`<meta property="og:image" content="${escapeHtml(state.ogImage)}" />`);
		if (state.siteUrl)
			lines.push(`<meta property="og:url" content="${escapeHtml(state.siteUrl)}" />`);
	...
	const twitterTags = useMemo(() => {
		...
		if (state.title)
			lines.push(`<meta name="twitter:title" content="${escapeHtml(state.title)}" />`);
		if (state.description)
			lines.push(`<meta name="twitter:description" content="${escapeHtml(state.description)}" />`);
		if (state.ogImage)
			lines.push(`<meta name="twitter:image" content="${escapeHtml(state.ogImage)}" />`);
		if (state.twitterHandle)
			lines.push(`<meta name="twitter:site" content="${escapeHtml(state.twitterHandle)}" />`);
		return lines.join("\n");
	}, [state]);
		...
		<div>
			...
			<div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
				<div>
					...
					/>
					{titleWarning && (
						<p class="text-caption text-accent-rose mt-1">
						...
						<div class="flex items-center gap-2 mb-1">
							...
							<div>
								<div class="text-body-sm" style="color: #bdc1c6">
									{state.siteUrl
										? state.siteUrl.replace(/^https?:\/\//, "").replace(/\/$/, "")
										: "example.com"}
								</div>
							</div>
						</div>
