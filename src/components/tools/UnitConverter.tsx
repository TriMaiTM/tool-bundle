				...
				<div class="grid grid-cols-1 lg:grid-cols-[1fr_auto_1fr] gap-4 items-end">
					...
					<div>
						...
						<select
							...
							value={fromUnit}
							onChange={(e) => setFromUnit(Number.parseInt((e.target as HTMLSelectElement).value))}
						>
							{units.map((u, i) => (
					...
					<div>
						...
						<select
							class="input w-full mb-3"
							value={toUnit}
							onChange={(e) => setToUnit(Number.parseInt((e.target as HTMLSelectElement).value))}
						>
							{units.map((u, i) => (
								<option key={u.abbr} value={i}>
									{u.name} ({u.abbr})
				...
				{result !== null && inputValue.trim() !== "" && (
					<div class="mt-6 bg-surface-elevated rounded-lg p-3 text-center border border-hairline">
						<div class="text-title-lg text-primary">
							{formatResult(Number.parseFloat(inputValue))} {units[fromUnit].abbr} ={" "}
							{formatResult(result)} {units[toUnit].abbr}
						</div>
					</div>
				)}
			</div>
								...
								{[1, 5, 10, 25, 50, 100, 1000].map((val) => (
									<tr key={val} class="border-b border-hairline/50">
										...
										<td class="py-2 px-3 text-right text-primary">
											{formatResult(
												convert(category, val, units[fromUnit].abbr, units[toUnit].abbr),
											)}
										</td>
									</tr>
								))}
