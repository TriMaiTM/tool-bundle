...
export default function CssGradientGenerator() {
	...
	const addStop = useCallback(() => {
		...
		setStops((prev) =>
			[...prev, { color: "#ffffff", position: 50 }].sort((a, b) => a.position - b.position),
		);
	}, [stops.length]);
					...
					{gradientType === "linear" && (
						<div class="flex-1 min-w-[200px]">
							...
							<input
								...
								max="360"
								value={angle}
								onInput={(e) => setAngle(Number.parseInt((e.target as HTMLInputElement).value))}
								class="w-full"
							/>
					...
					{gradientType === "radial" && (
						<div>
							...
							<div class="flex gap-2">
								...
								<button
									class={
										radialShape === "ellipse" ? "btn-primary text-sm" : "btn-secondary text-sm"
									}
						...
						<div class="flex items-center gap-3 p-3 rounded-lg bg-surface-soft">
							...
							<div class="flex-1">
								...
								<input
									...
									value={stop.position}
									onInput={(e) =>
										updateStopPosition(index, Number.parseInt((e.target as HTMLInputElement).value))
									}
									class="w-full"
								/>
							</div>
